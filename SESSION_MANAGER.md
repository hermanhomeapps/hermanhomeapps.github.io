...
- [ ] **Paste the LATEST `hermanhomeapps-backend.gs` again and redeploy** — it now uses `SpreadsheetApp.openById(CHORES_SHEET_ID)` everywhere instead of `getActiveSpreadsheet()`, fixing a real crash (`TypeError: ...getSheetByName` at doGet/doPost) that was happening on every Chores/Camp request on the permanent URL, since that project isn't container-bound to the Chores Sheet. Frontend files are already repointed at the permanent URL (`AKfycbwJ...`) in this session's commits — this redeploy is the only remaining piece to make Mommy Camp/Chores actually work end-to-end there.
- [ ] Once confirmed working: optionally delete the old, now-unreferenced Chores-bound Apps Script project in script.google.com (whatever was serving the `AKfycbx2...` URL) — nothing points at it anymore.
